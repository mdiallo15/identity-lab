  ...
  return (
    <>
      ...
      <p className="lede">
        ...
        <em>great, but my AI agent has an OpenAI API key in a config file —
        how is that any better than a password?</em> It's not. The fix uses
        the same building blocks as WebAuthn, applied to workloads.
      </p>

      <h2>Three identities, not one</h2>
      <p>
        When an agent runs a task on your behalf, three identities are in play.
        Most platforms collapse them and that's where the security model
        breaks.
      </p>
      <ul className="triple">
        <li>
        ...
        <li>
          <strong>Agent identity.</strong> Which agent — which model
          deployment, which container, which version — is running. This is a
          workload identity, not a human one.
        </li>
        <li>
      ...
      <dl>
        ...
        <dd>
          ...
          tokens to running workloads. The token is short-lived, attested by
          the platform, and the relying party verifies it the same way it
          verifies a human's ID token. No long-lived API key needed.
          <br />
        ...
        <dd>
          ...
          UID, k8s service account, hardware TPM). Used by Bloomberg,
          Spotify, and most large platforms running mTLS at scale.
        </dd>
        ...
        <dd>
          OAuth 2.0 token exchange (RFC 8693) lets a user's passkey-issued
          token be downgraded into a narrower, time-boxed token for the
          agent. The agent's token is bound to its workload identity via
          DPoP (RFC 9449), so a stolen token can't be used elsewhere.
        </dd>
      </dl>
      ...
      <ol className="bad-models">
        <li>
          ...
          key for the LLM provider, plus another long-lived token for every
          tool it calls (GitHub PAT, calendar token, Stripe key). Steal the
          agent's container, get keys to everything. This is most agents in
          production right now.
        </li>
        <li>
          <strong>Impersonation model.</strong> The agent uses{" "}
          <em>your</em> credentials directly. Whatever you can do, the agent
          can do, with no audit distinction. Your access logs say "user did
          X" when the agent did X.
        </li>
      </ol>
      ...
      <ol className="action">
        <li>
          User authenticates with a passkey → receives a session token at
          AAL2 / AAL3.
        </li>
        <li>
          User invokes agent → token-exchange (RFC 8693) downgrades the
          session into a narrower agent token: scope ={" "}
      ...
      <p>
        This is what OIDC's <code>act</code> claim and OAuth 2.0 Token
        Exchange were designed for. Almost nobody uses them yet for agents.
        That's the gap.
      </p>
      ...
      <ul className="gotchas">
        ...
        <li>
          <strong>Bound scopes per task.</strong> The agent doing a code
          review doesn't need <code>repo:write</code>. Token-exchange every
          tool call to the minimum scope required.
        </li>
        <li>
